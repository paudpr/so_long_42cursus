...
void	fill_img(char code, t_mlx *mlx, t_coord coord, t_sprites *sprites)
{
	if (code == '0' || code == 'C' || code == 'E' || code == 'P'
		|| code == 'X')
		mlx_put_image_to_window(mlx->mlx, mlx->win, sprites->floor, coord.x * 32, coord.y * 32);
	if (code == '1')		
		mlx_put_image_to_window(mlx->mlx, mlx->win, sprites->wall, coord.x * 32, coord.y * 32);
	if (code == 'C')		
		mlx_put_image_to_window(mlx->mlx, mlx->win, sprites->collect, coord.x * 32, coord.y * 32);
	if (code == 'E')		
		mlx_put_image_to_window(mlx->mlx, mlx->win, sprites->exit, coord.x * 32, coord.y * 32);
	if (code == 'P')		
		mlx_put_image_to_window(mlx->mlx, mlx->win, sprites->mario, coord.x * 32, coord.y * 32);
}
